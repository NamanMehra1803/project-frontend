...
function User() {
    ...
    const Userview = async () => {
        try {
            const response = await axios.post('https://my-backend-api-usbu.onrender.com/userview');
    ...
    const searchUser = async (e) => {
        e.preventDefault();
        try {
            const response = await axios.post('https://my-backend-api-usbu.onrender.com/userSearch', { firstName, email });
        ...
        try {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    await axios.post('https://my-backend-api-usbu.onrender.com/api/admin/userdelete', { _id: user_Id });
    ...
    const handleStatusUpdate = async (user_Id) => {
        try {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    await axios.post('https://my-backend-api-usbu.onrender.com/statusUpdate', { _id: user_Id });
                                        ...
                                        {userData.length === 0 ? <tr><td colSpan="8">No users found.</td></tr> :
                                            currentItems.map((user, index) => (
                                                <tr key={user._id}>
                                                    ...
                                                    <td><img src={`https://my-backend-api-usbu.onrender.com/uploads/${user.image}`} alt={user.firstName || "No image"} style={{ height: '40px', width: '40px', borderRadius: '20%', objectFit: 'cover' }} /></td>
