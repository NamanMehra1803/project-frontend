...
export default function Cart() {
  ...
  const viewCart = async (user_id) => {
    try {
      const response = await axios.get(
        `http://localhost:8080/view-cart/${user_id}`
      );
  ...
  const removeFromCart = async (cartItemId) => {
    ...
    try {
      await axios.post('http://localhost:8080/remove-from-cart', { _id: cartItemId });
      Swal.fire({
        icon: "success",
        title: "Removed!",
...
const placeOrder = async () => {
  ...
  try {
    ...
    for (const item of cartItems) {
      await axios.post("http://localhost:8080/remove-from-cart", { _id: item._id });
    }
    viewCart(user_id);
  } catch (error) {
